...
const GoalGauge = () => {
    ...
    const percent =
        goalTarget > 0 ? Math.min((goalTotal / goalTarget) * 100, 100) : 0;
    ...
    return (
        <div className="w-80 rounded-xl bg-black/55 px-4 py-3 text-white backdrop-blur">
            ...
            <div className="h-3 overflow-hidden rounded-full bg-white/15">
                <div
                    className="h-full rounded-full bg-linear-to-r from-violet-400 to-fuchsia-500 transition-[width] duration-700 ease-out"
                    style={{ width: `${percent}%` }}
                />
            </div>
